...
def test_add_and_search(tmp_path: Path):
    store = FaissStore(persist_directory=str(tmp_path), embedding_dimension=384)
...
def test_persistence_survives_reload(tmp_path: Path):
    ...
    del store
    store2 = FaissStore(persist_directory=str(tmp_path), embedding_dimension=384)
    assert store2.get_stats()["records"] == 1
    results = store2.similarity_search(rec["embedding"], top_k=1)
    assert results[0]["id"] == "c1"


def test_delete_document_rebuilds_index(tmp_path: Path):
    store = FaissStore(persist_directory=str(tmp_path), embedding_dimension=384)
    store.add_documents([
    ...
    results = store.similarity_search(
        _make_record("q", "q", "other")["embedding"], top_k=5
    )
    assert len(results) == 1
